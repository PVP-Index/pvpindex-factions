...
@DisplayName("CmdRelationWishes — /f relation wishes")
class CmdRelationWishesTest extends CommandTestBase {
    ...
    @BeforeEach
    void setUp() {
        cmd = new CmdRelationWishes(factionService);
    ...
    @StorageTest
    @DisplayName("no relation wishes — empty message shown")
    void testNoWishes() {
        ...
        verify(player).sendMessage(argThat(componentContains("No relation wishes")));
    }


    @StorageTest
    @DisplayName("has relation wishes — redirect message shown")
    void testHasWishes() {
        when(factionService.getFactionByPlayer(uuid)).thenReturn(Optional.of(faction));
        when(faction.getRelationsJson()).thenReturn("{\"some-id\":\"ALLY\"}");


        cmd.execute(ctx());


        verify(player).sendMessage(argThat(componentContains("relation list")));
    }
}
