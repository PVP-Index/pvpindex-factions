...
/** {@code /fa safezone [one|square|circle|remove] [radius]}. */
public final class CmdAdminSafezone extends FactionCommand {

    public CmdAdminSafezone() {
        super("safezone");
        setPermission("factions.cmd.safezone");
        setDescription("Assign or remove safe zone territory chunks.");
        ...
        if ("remove".equals(mode)) {
            ...
            try {
                ...
                if (existing.isPresent()
                        && FactionModel.SAFEZONE_ID.equals(existing.get().getFactionId())) {
                    ctx.getRepos().board().unclaimChunk(
                        chunk.getWorld().getName(), chunk.getX(), chunk.getZ());
                    MsgUtil.send(player, "<green>Removed safe zone from this chunk.");
                } else {
                    MsgUtil.send(player, "<red>This chunk is not a safe zone.");
                }
            } catch (Exception e) {
                MsgUtil.send(player, "<red>Failed to remove safe zone chunk.");
            }
            return;
        }
        ...
        for (final Chunk chunk : targets) {
            try {
                ctx.getRepos().board().claimChunk(
                    chunk.getWorld().getName(), chunk.getX(), chunk.getZ(),
                    FactionModel.SAFEZONE_ID);
                assigned++;
            } catch (Exception ignored) {
                // Keep processing remaining chunks.
            }
        }
        MsgUtil.send(player, "<green>Assigned <white>" + assigned + "<green> chunk(s) as safe zone.");
    }
